					...
					<ul class="board-list">
							...
							<li>
								...
								<c:choose>
									...
									<li class="board-title-con txt-l">
										<a href="/front/notification/detail?noti_seq=${list.noti_seq}" style="color: inherit;">${list.noti_title}</a>
									</li>
									<li>
						...
						<ul class="pagination justify-content-center">
							<c:if test="${pageMaker.prev}">
								<li class="page-item"><a class="page-link" href='<c:url value="/front/notification/list?page=${pageMaker.startPage-1}"/>'>prev</a></li>
							</c:if>
								...
								<c:choose>
									<c:when test = "${pageMaker.cri.page == pageNum}">
										<li class="page-item active" aria-current="page">
											<a class="page-link noti_a" href='<c:url value="/front/notification/list?page=${pageNum}"/>'>${pageNum}</a>
										</li>
									</c:when>
									<c:otherwise>
										<li class="page-item ">
											<a class="page-link" href='<c:url value="/front/notification/list?page=${pageNum}"/>'>${pageNum}</a>
										</li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<c:if test="${pageMaker.next && pageMaker.endPage >0}">
								<li class="page-item"><a class="page-link" href='<c:url value="/front/notification/list?page=${pageMaker.endPage+1}"/>'>next</a></li>
							</c:if>
						</ul>
        </main>
